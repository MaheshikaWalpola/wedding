...
function drawStrands(svg) {
  ...
  function render() {
    ...
    svg.innerHTML = "";
    const wide = W >= 120; // three lanes on wide screens; one slim lane in the phone margins
    const lanes = wide ? [[28, 0.62], [74, 0.92], [120, 0.76]] : [[W * 0.36, 0.95]];
    const k1 = wide ? 1 : 0.75; // flower scale on the slim lane
    lanes.forEach(([x, f], j) => {
      const len = H * f;
      ...
      for (let y = 12, k = 0; y < len - 14; y += 13, k++) {
        const dx = (j % 2 ? 1 : -1) * Math.sin((y / len) * Math.PI) * 7 * k1;
        if (k % 5 === 4) g.appendChild(use("araliya", dx, y - 8 * k1, 16 * k1, 16 * k1, { transform: `rotate(${(k * 40) % 360} ${dx} ${y})` }));
        else if (k % 3 === 2) g.appendChild(use("jasmine", dx, y - 5.5 * k1, 11 * k1, 11 * k1));
        else g.appendChild(use("bud", dx, y - 1, 7 * k1, 12 * k1, { transform: `rotate(${k % 2 ? 22 : -22} ${dx} ${y})` }));
      }
      g.appendChild(use("jasmine", -6 * k1, len - 10, 11 * k1, 11 * k1)); g.appendChild(use("jasmine", 6 * k1, len - 7, 10 * k1, 10 * k1)); g.appendChild(use("araliya", 0, len - 6, 18 * k1, 18 * k1));
      svg.appendChild(pos);
    });
  }
  render();
...
function setupPetals() {
  ...
  function size() {
    ...
    if (widthChanged || !petals.length) {
      const n = W < 700 ? 44 : Math.round(Math.min(30, Math.max(14, W / 50))); // phones get a fuller fall
